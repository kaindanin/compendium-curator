...
function getRefreshSectionTitle(key, count) {
    const text = game.i18n.format(`COMPENDIUM_CURATOR.${key}`, {
        count
    });
    return text
        .replace(/\s*\(\s*\d+\s*\)\s*$/, "")
...
function renderRefreshDocumentList(title, count, entries) {
    const escape = foundry.utils.escapeHTML;
    const resultsLabel = escape(game.i18n.localize("DND5E.CompendiumBrowser.Column.Results"));
    const sourceLabel = escape(game.i18n.localize("DND5E.CompendiumBrowser.Column.Source"));
    const rows = entries
        .map(entry => {
        const uuid = escape(String(entry.uuid ?? ""));
        const name = escape(String(entry.name ??
            entry.uuid ??
            ""));
        const img = entry.img
        ...
        const subtitle = entry.subtitle
            ? escape(game.i18n.localize(entry.subtitle))
            : "";
...
export class TableManagerApplication extends HandlebarsApplicationMixin(ApplicationV2) {
    ...
    async _prepareContext(options) {
        ...
        await TableProfileStorageService
            .migrateStorage();
        const profiles = Object.values(TableProfileStorageService
            .getProfiles())
            ...
            const filterGroups = Array.isArray(profile.filterGroups)
                ? profile.filterGroups
                    ...
                    .sort((a, b) => String(a.name ?? "")
                    .localeCompare(String(b.name ?? ""), game.i18n.lang, {
                    sensitivity: "base"
                }))
                : [];
            ...
            const childCount = Array.isArray(profile.children)
                ...
                : 0;
            const typeLabel = game.i18n.localize(isNested
            ...
            const summary = isNested
                ? game.i18n.format("COMPENDIUM_CURATOR.SubtableCount", {
                    count: childCount
                })
                : game.i18n.format("COMPENDIUM_CURATOR.FilterGroupCount", {
                    count: filterGroupCount
                });
            const revision = Number(profile.revision ?? 1);
            const generatedRevision = Number(profile
                .generation
                ?.generatedRevision ??
                0);
            ...
            else if (generatedRevision > 0 &&
                generatedRevision ===
                    revision) {
                statusKey =
                    "TableProfileUpToDate";
            }
            const status = game.i18n.localize(`COMPENDIUM_CURATOR.${statusKey}`);
            return {
                id: profile.id,
                name: profile.name,
                type,
        ...
        const contentProfiles = profiles.filter(profile => profile.isContent);
        const nestedProfiles = profiles.filter(profile => profile.isNested);
        const filterGroups = Object.values(TableProfileStorageService
            .getFilterGroups())
            .map(filterGroup => {
            const usedBy = profiles
                .filter(profile => profile.filterGroups
                .some(group => group.id ===
                filterGroup.id))
                .map(profile => ({
                id: profile.id,
            ...
            return {
                ...
                useCount,
                usedBy,
                matchesLabel: game.i18n.format("COMPENDIUM_CURATOR.CurrentFilterMatches", {
                    count: matchCount
                }),
                usageLabel: `${useCount} ${game.i18n.localize("COMPENDIUM_CURATOR.TableProfiles").toLocaleLowerCase()}`,
                searchText: [
                    filterGroup.name,
        ...
        const isContentTab = this._activeTab ===
            "content";
        const isNestedTab = this._activeTab ===
            "nested";
        const isFilterGroupsTab = this._activeTab ===
            "filters";
        context.isContentTab =
            isContentTab;
        context.isNestedTab =
            isNestedTab;
        ...
        context.searchLabel =
            this.browserApp
                ...
                ?.placeholder ??
                game.i18n.localize("COMPENDIUM_CURATOR.TableProfiles");
        return context;
    }
    async _onRender(context, options) {
        await super._onRender(context, options);
        const input = this.element.querySelector('[name="managerSearch"]');
    ...
    _applyManagerSearch() {
        ...
        for (const entry of this.element.querySelectorAll("[data-cc-search-text]")) {
            const haystack = normalizeManagerSearchText(entry.dataset
                .ccSearchText);
            entry.hidden =
                Boolean(query) &&
    ...
    async _preClose(options) {
        ...
        if (this.browserApp) {
            ...
            if (this.browserApp._ccTableManager ===
                this) {
    ...
    static #onChangeManagerTab(event, target) {
        ...
        const tab = String(target.dataset
            ?.tab ??
            "");
        if (![
            "content",
    ...
    static async #onAddCurrentFilters(event, target) {
        ...
        this._filterGroupEditor =
            new TableFilterGroupApplication(this.browserApp, this, profileId);
    ...
    static async #onPreviewProfile(event, target) {
        ...
        this._profilePreview =
            new TableProfilePreviewApplication(this.browserApp, profileId);
        this._profilePreview.render({
            force: true
        });
    }
    static async #onManualInclusions(event, target) {
        ...
        this._profileInclusions =
            new TableProfileInclusionsApplication(this.browserApp, this, profileId);
        this._profileInclusions.render({
            force: true
        });
    }
    static async #onManualExclusions(event, target) {
        ...
        this._profileExclusions =
            new TableProfileExclusionsApplication(this.browserApp, this, profileId);
        this._profileExclusions.render({
            force: true
        });
    }
    static async #onRenameProfile(event, target) {
        ...
        label.textContent =
            game.i18n.localize("COMPENDIUM_CURATOR.ProfileName");
        ...
        const result = await foundry
            ...
            .input({
            window: {
                title: game.i18n.localize("COMPENDIUM_CURATOR.RenameProfile")
            },
        ...
        if (!name) {
            ui.notifications.warn(game.i18n.localize("COMPENDIUM_CURATOR.ProfileNameRequired"));
            return;
        }
        if (name === profile.name)
            return;
        if (TableProfileStorageService
            .isNameTaken(name, profileId)) {
            ui.notifications.warn(game.i18n.localize("COMPENDIUM_CURATOR.TableProfileNameTaken"));
            return;
        }
        await TableProfileStorageService
    ...
    static async #onDuplicateProfile(event, target) {
        ...
        if (!profile)
            return;
        let suggestedName = game.i18n.format("COMPENDIUM_CURATOR.ProfileCopyName", {
        ...
        const result = await foundry
            ...
            .input({
            window: {
                title: game.i18n.localize("COMPENDIUM_CURATOR.DuplicateProfile")
            },
            content: field.outerHTML,
            ok: {
    ...
    static async #onDeleteProfile(event, target) {
        ...
        const confirmed = await foundry
            ...
            window: {
                title: game.i18n.localize("COMPENDIUM_CURATOR.DeleteProfile")
            },
            content: `<p>${game.i18n.format("COMPENDIUM_CURATOR.ProfileDeleteConfirm", {
    ...
    static #onToggleProfileActions(event, target) {
        event.preventDefault();
        event.stopPropagation();
        const profile = target.closest(".cc-table-manager-profile");
        if (!profile)
            return;
    ...
    _openProfileActionsPopover(profile, anchor) {
        this._closeProfileActionsPopover();
        const sourceMenu = profile.querySelector(".cc-table-manager-profile-menu");
        if (!sourceMenu)
            return;
        ...
        popover.addEventListener("click", event => {
            ...
            const originalButton = profile.querySelector(`.cc-table-manager-profile-menu
        ...
        document.addEventListener("pointerdown", this._profileActionsOutsideHandler, true);
        document.addEventListener("scroll", this._profileActionsViewportHandler, true);
        window.addEventListener("resize", this._profileActionsViewportHandler);
    }
    ...
    _closeProfileActionsPopover() {
        if (this._profileActionsOutsideHandler) {
            document.removeEventListener("pointerdown", this._profileActionsOutsideHandler, true);
        }
        if (this._profileActionsViewportHandler) {
            document.removeEventListener("scroll", this._profileActionsViewportHandler, true);
            window.removeEventListener("resize", this._profileActionsViewportHandler);
        }
        this._profileActionsPopover
            ?.remove();
        this._profileActionsPopover =
    ...
    static async #onFilterGroupDetails(event, target) {
        ...
        this._filterGroupDetails =
            new TableFilterGroupDetailsApplication(this.browserApp, profile, filterGroup);
        this._filterGroupDetails.render({
            force: true
        });
    }
    static async #onRefreshFilterGroup(event, target) {
        ...
        const filters = TableProfileService
            .compactBrowserFilters(filterGroup
            ?.browser
            ?.filters ??
            {});
        if (!filters)
            return;
        const currentCandidates = await TableProfileService
            .getBrowserCandidates(this.browserApp, filters);
        ...
        if (!Array.isArray(filterGroup.matches)) {
            const confirmed = await foundry
                ...
                classes: TABLE_DIALOG_CLASSES,
                window: {
                    title: game.i18n.localize("COMPENDIUM_CURATOR.RefreshFilterGroup")
                },
                content: `<p>${game.i18n.format("COMPENDIUM_CURATOR.InitializeFilterGroupMatches", {
                    name: foundry.utils.escapeHTML(filterGroup.name),
            ...
            if (!confirmed)
                return;
            await TableProfileStorageService
                .updateFilterGroupMatches(profileId, filterGroupId, currentByUuid.keys(), filters);
            ui.notifications.info(game.i18n.localize("COMPENDIUM_CURATOR.FilterGroupRefreshed"));
            this.render({
                force: true
            });
            this._refreshApplicationsForFilterGroup(filterGroupId);
        ...
        ].filter(candidate => !previous.has(candidate.uuid));
        const removedUuids = [...previous].filter(uuid => !currentByUuid.has(uuid));
        if (added.length === 0 &&
            removedUuids.length === 0) {
            await TableProfileStorageService
                .updateFilterGroupMatches(profileId, filterGroupId, currentByUuid.keys(), filters);
            ui.notifications.info(game.i18n.localize("COMPENDIUM_CURATOR.NoFilterGroupChanges"));
            return;
        }
        const addedEntries = await prepareDnd5eDocumentEntries(added.map(candidate => candidate.uuid));
        const removedEntries = await prepareDnd5eDocumentEntries(removedUuids);
        const addedHtml = added.length
            ? renderRefreshDocumentList(getRefreshSectionTitle("NewMatches", added.length), added.length, addedEntries)
            : "";
        const removedHtml = removedUuids.length
            ? renderRefreshDocumentList(getRefreshSectionTitle("RemovedMatches", removedUuids.length), removedUuids.length, removedEntries)
            : "";
        const confirmed = await foundry
            .applications
            .api
            ...
            classes: TABLE_DIALOG_CLASSES,
            window: {
                title: game.i18n.format("COMPENDIUM_CURATOR.RefreshFilterGroupTitle", {
                    name: filterGroup.name
                })
            },
            position: {
    ...
    static async #onLoadFilterGroup(event, target) {
        ...
        const loaded = await TableProfileService
            .loadBrowserFilters(this.browserApp, filterGroup.browser);
        if (loaded === null)
            return;
        if (loaded === false) {
            ui.notifications.warn(game.i18n.localize("COMPENDIUM_CURATOR.FilterGroupFiltersLoadFailed"));
            return;
        }
